...
function selectQuestion(list, qntQuestion) {
    return list = list.slice(0, qntQuestion);
}

function seleciona10Questoes(questoes, qntQuestion) {
    let newQuestion = questoes;
    for (let i = newQuestion.length - 1; i > 0; i--) {
        let j = Math.floor(Math.random() * (i + 1));
        [newQuestion[i], newQuestion[j]] = [newQuestion[j], newQuestion[i]];
    }
    newQuestion = selectQuestion(newQuestion, qntQuestion);
    return newQuestion;
}

function validarResposta(respostaUsuario, pergunta) {
...
function iniciarProg() {
    ...
    let nome = "Rafael"; //entradaDados.question('Nome Completo: '); 
    let qntQuestion = 10 //Number(entradaDados.question(`Quantas Perguntas: `))
...
---------- Seja Bem-vindo ${nome}`);
    ...
    let questoesSelect = [...questoes];
    questoesSelect = seleciona10Questoes(questoesSelect, qntQuestion); 

    let pontuacao = 0;

    questoesSelect.forEach((pergunta, index) => {
    ...
    exibirResultado(pontuacao, nome);
}
